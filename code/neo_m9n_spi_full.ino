...
#define LORA_NSS_PIN  21       // set to the SX1262 NSS GPIO so it releases the shared MISO; -1 to skip
...
void setup() {
    ...
    // Enable UBX-NAV-PVT streaming on the SPI port (CFG-MSGOUT-UBX_NAV_PVT_SPI = 1).
    // CFG-VALSET = class 0x06 id 0x8A. Payload: version, layers, reserved(2), key(4 LE), value.
    // layers = 0x01 (RAM only, reverts on power cycle). Use 0x07 for RAM|BBR|Flash to persist.
    static const uint8_t cfgPvtSpi[] = {
        0x00,                     // version
        0x01,                     // layers: RAM only
        0x00, 0x00,               // reserved
        0x0A, 0x00, 0x91, 0x20,   // key 0x2091000A, little-endian
        0x01                      // value: rate 1 = output every nav epoch
    };
    sendUBX(0x06, 0x8A, cfgPvtSpi, sizeof(cfgPvtSpi));   // expect ACK for cls=0x06 id=0x8A
